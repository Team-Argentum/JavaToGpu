...
`summary` is the default and emits compact aggregate counters, including proof-bundle rewrite-safety and diagnostic counts such as `autoVectorizationProofBundleRewriteSafe` and `autoVectorizationProofBundleDiagnostics`. `quiet` suppresses diagnostic notes while still running the read-only pipeline. `detailed` emits nested optimizer context, including `autoVectorizationProofBundle={...}`, and is intended for CI/debug runs where verbose javac output is acceptable.
