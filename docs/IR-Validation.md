...
`cseLayerReadiness*` provides the matching compact CSE layer view on the read-only snapshot. It reports the layer order, present layers, blocking layers, first blocking layer, per-layer states, and `CiSummaryLine` across rewrite policy, skip reason, dominance, local-expression, and simple-arithmetic proof surfaces, so CI can see whether CSE is `ready`, `blocked`, or has `noRewriteWork` without parsing raw counters.

`optimizerLayerReadiness*` sits above the main validation report as a compact read-only optimizer-layer summary. It rolls up safety, optimizer gate, CSE, CSE literal-promotion, auto-vectorization proof-layer, and auto-vectorization readiness into a single artifact surface with verdict, present/blocking layers, first blocker, per-layer states, and `CiSummaryLine` fields. Detached tooling can consume the same surface directly through `GpuIrOptimizationValidationOptimizerEnablementArtifactRunner.runOptimizerLayerReadinessFields(...)` or `GpuIrOptimizationValidationProductionEnablementReadinessRunner.runOptimizerLayerReadinessFields(...)`, and the full `optimizerValidationBundle*` export includes bundle-level layer-readiness shortcut fields plus the nested `optimizerLayerReadiness*` map. `optimizerLayerReadinessConsistency*` self-check fields verify that verdict, blocking flags, counters, first blocker, and per-layer states remain internally coherent before CI consumes the rollup.
`optimizerLayerReadinessRegression*` compares a baseline readiness snapshot with a current validation report and exports `improved`, `regressed`, `unchanged`, layer deltas, changed/improved/regressed layer lists, per-layer transitions, and a CI summary line. The detached enablement and production-readiness runners expose this through `runOptimizerLayerReadinessRegressionFields(...)` for CI dashboards that track L2 progress over time. For CI jobs that want pass/warn/fail semantics instead of raw comparison fields, the same baseline/current pair can be evaluated through `runOptimizerLayerReadinessRegressionRuleArtifactFields(...)`, which exports the standard `validationRules*` artifact around `optimizer.layerReadinessRegressionGate`: unchanged snapshots pass, improvements warn without blocking, and regressions fail/block.
...
Side-effecting auto-vectorization values are now represented twice on purpose: the first blocking diagnostic still reports `SIDE_EFFECTING_VALUE` for readable javac output, and the proof bundle also exports typed `sideEffect` evidence with `blockedBySideEffect` / `autoVectorizationProofBundleGuardFamily.sideEffect` fields for CI and future rewrite gates.

Alias and neighboring mutation blockers are similarly split into readable rewrite-plan guard diagnostics plus typed proof evidence. `GpuIrAutoVectorizationMutationProofReport` groups `targetSourceAlias`, `neighborSourceWrite`, and `neighborTargetWrite` guards into the `mutation` proof kind, so artifacts can distinguish mutation blockers without double-counting guard-family totals.

Backend and device capability blockers are split the same way. `GpuIrAutoVectorizationBackendProofReport` groups `backendVectorWidth` and `backendDoubleVector` guards into the `backend` proof kind, preserving existing rewrite-plan diagnostics while giving CI and future rewrite gates a typed backend-readiness layer.

Unknown-vector blockers are split into the same typed proof layer. `GpuIrAutoVectorizationUnknownVectorProofReport` groups `unknownVectorType` guards into the `unknownVector` proof kind, preserving existing rewrite-plan diagnostics while giving CI and future rewrite gates a typed unknown-type-readiness layer.

The proof layers also have a compact readiness rollup. `GpuIrAutoVectorizationProofLayerReadinessSummaryReport` exports `autoVectorizationProofLayerReadiness*` fields for layer order, present layers, blocking layers, first blocking layer, per-layer states, and a CI summary line across `unknownVector`, `backend`, `mutation`, `controlFlowBoundary`, `memoryLegality`, and `sideEffect`.
...
`GpuIrOptimizationValidationOptimizerEnablementGateReport` is the first read-only aggregate gate above those artifacts. It combines CSE literal-promotion readiness, auto-vectorization prototype readiness, and the conservative optimizer enablement policy into one `optimizerEnablementGate*` verdict with blocker counts, first blocker, remaining work, and CI summary text. CSE now separates `CseReadyForEnablementReview` from `CseReadyForProductionMutation`, so successful runtime-equivalence evidence can advance to `reviewReady/productionMutationDisabled` while production mutation remains explicitly disabled. A blocked CSE or auto-vectorization readiness layer still fails the gate before policy review; a clean review reports `reviewReady/productionMutationDisabled` until a future explicit production mutation switch is implemented.
...
`GpuIrOptimizationValidationProductionEnablementReadinessRunner` is the explicit A1/A2 opt-in runner for that final preflight. It accepts an already-created validation report, returns the production preflight decision or full bundle fields, and keeps the whole path read-only so CI can ask for production-enable readiness without changing compiler behavior.

`GpuIrOptimizationValidationProductionMutationSwitchContract` is the read-only contract for a future production optimizer mutation switch. It sits above the A1/A2 preflight, exports `optimizerProductionSwitch*` fields, and separates switch-review eligibility from actual mutation enablement. The contract records required evidence such as stable CPU/GPU runtime equivalence, cross-vendor runtime coverage, a rollback strategy, and an explicit production mutation feature flag. It does not enable mutation; it only tells tooling whether the current preflight is blocked, review-required with production mutation disabled, or future-ready in synthetic tests.

`GpuIrOptimizationValidationOptimizerPromotionConfidenceContract` is the final read-only promotion-confidence gate above the production switch contract. It exports `optimizerPromotionConfidence*` fields and keeps selected optimizer rewrite promotion blocked until A1/A2 runtime-equivalence history, long-running operational stress, NVIDIA interim coverage, cross-vendor promotion gates, and rollback evidence are stable. This keeps CSE and auto-vectorization rewrites reviewable without enabling production mutation or promotion by default.

`GpuIrOptimizationValidationProductionReadinessArtifact` is the CI-facing aggregate above the whole production-readiness chain. It packages the validation bundle, production preflight decision, future production-switch contract, and promotion-confidence contract into one `optimizerProductionReadiness*` surface with the top-level verdict, blocker list, remaining work, production-mutation state, nested preflight/switch/promotion fields, consistency self-check fields, acceptance-decision fields, and a compact CI summary line. The detached readiness runner exposes it through `runProductionReadinessArtifact(...)`, `runProductionReadinessArtifactFields(...)`, `runProductionReadinessArtifactAcceptance(...)`, and `runProductionReadinessArtifactAcceptanceFields(...)` without enabling mutation.
...
Compiled-method auto-vectorization memory proof now also fails closed when a target or source array lacks parameter metadata. Partially typed candidates remain visible for diagnostics, but rewrite operations are blocked with `memoryAddressSpace` guards; fully unknown lane-array element types still reject as `UNSUPPORTED_ELEMENT_TYPE` before rewrite preview.

Auto-vectorization side-effect blockers now have the same typed proof treatment: `GpuIrAutoVectorizationSideEffectProofReport` converts `SIDE_EFFECTING_VALUE` rejections into proof summaries, so future rewrite enablement can branch on `sideEffect` / `blockedBySideEffect` instead of parsing human-readable rejection strings.

Auto-vectorization alias and neighboring mutation blockers now also expose a dedicated typed proof layer. `GpuIrAutoVectorizationMutationProofReport` derives `mutation` proof summaries from existing rewrite-plan guards, keeps first blocking diagnostics unchanged, and avoids double-counting guard-family counters that are already present in the rewrite-plan proof.

Auto-vectorization backend/device blockers now expose the same dedicated typed proof layer. `GpuIrAutoVectorizationBackendProofReport` derives `backend` proof summaries from existing `backendVectorWidth` and `backendDoubleVector` rewrite-plan guards, keeps first blocking diagnostics unchanged, and avoids double-counting guard-family counters that are already present in the rewrite-plan proof.

Auto-vectorization unknown-vector blockers now expose the same dedicated typed proof layer. `GpuIrAutoVectorizationUnknownVectorProofReport` derives `unknownVector` proof summaries from existing `unknownVectorType` rewrite-plan guards, keeps first blocking diagnostics unchanged, and avoids double-counting guard-family counters that are already present in the rewrite-plan proof.

Auto-vectorization proof-layer readiness now has its own compact summary surface. `GpuIrAutoVectorizationProofLayerReadinessSummaryReport` is exported through `GpuIrAutoVectorizationArtifactSnapshot`, so CI can read `autoVectorizationProofLayerReadinessVerdict`, `BlockingLayers`, `FirstBlockingLayer`, and `Layer.*` fields directly instead of reconstructing typed layer state from raw proof-bundle counters.
