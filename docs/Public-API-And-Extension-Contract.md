...
Non-analysis optimizer-family payloads are indexed by `runtime-optimizer-family-equivalence-payload.properties`. For each family/pass, runtime artifact dumping materializes a manifest plus CPU/reference, pre-optimization output, post-optimization output, tolerance, failure-fixture, and diagnostics properties files under a normalized family/pass directory. Runtime-equivalence reports may attach immutable per-case inputs, raw reference/pre/post outputs, tolerances, equivalence flags, and diagnostics. The dumper copies each structured field into its relevant component file and preserves the complete deterministic field set in the diagnostics file. Missing payload components remain explicit `not-recorded` artifacts. Artifact names must resolve inside the kernel artifact directory; absolute or parent-traversal paths are rejected.

`runtime-equivalence.properties` is the pipeline-level execution artifact. In addition to backend/vendor/profile status and aggregate counts, it may contain indexed raw comparison cases with an explicit comparison mode, invocation inputs, reference/candidate outputs, exact tolerance metadata, per-output equivalence flags, and diagnostics. OpenCL currently records descriptor-source versus reconstructed-`IrGpu` source comparisons. This artifact must not be copied into every optimizer-family payload unless a future executor runs and binds the comparison independently for that family.

The optimizer-family payload index exposes `familyBinding.status`, `familyBinding.eligible`, `familyBinding.family`, and `familyBinding.firstBlocker`. A binding is accepted only when runtime equivalence passed, exactly one optimizer family is present, raw comparison cases exist, and every comparison mode starts with `optimizer-family:<family>:`. Pipeline source-reconstruction comparisons therefore remain explicitly `not-bound`.
...
The production-promotion explainability artifact records controlled activation-token positive and negative hardware evidence. It exposes whether the token loaded, every approved real workload kernel executed, full workload coverage was reached, a mismatched artifact digest was rejected, an unapproved kernel was rejected before output mutation, and safe defaults remained intact. This evidence improves auditability but is not itself a production decision and cannot change source-switching or mutation authorization.
