...
Runtime peephole/InstCombine proof is also kept read-only. `GpuRuntimeIrPeepholeReplacementPlan` records future structural replacement intent, including root node, covered node ids, input node ids, replacement kind, completeness, and first blocker. `GpuRuntimeIrTypedNodeGraph` is the shared read-only traversal view for typed peephole rules, and `GpuRuntimeIrPeepholeReplacementPlanValidation` checks each emitted plan against that graph before it is surfaced. Validation fields such as `replacementPlan.validation.invalid.count` and `rule.N.replacementPlan.validation.firstBlocker` fail closed on missing roots, uncovered roots, or missing covered/input nodes; they still do not build replacement nodes or authorize mutation. The current matcher set covers `mad/fma` for `a*b+c` / `c+a*b`, `clamp` for `min(max(x, lo), hi)`, `step` for simple `x < edge ? 0 : 1` / `x >= edge ? 1 : 0` ternaries, `dot` for additive multiply trees such as `(a0*b0)+(a1*b1)`, and `mix` for `a + t * (b - a)` / `t * (b - a) + a`. `runtime-optimizer-drift.properties` aggregates complete/partial replacement-plan counts, structural plan-validation total/valid/invalid counts, first incomplete/invalid blockers, and compact `optimizerRule.*` summaries for CI drift detection. Workload gates, validation reports/history, I3 summaries, and production-promotion explainability carry those counters forward as evidence-only telemetry. The pass still reports `rewrite-engine-not-implemented` and never mutates the selected IR.
