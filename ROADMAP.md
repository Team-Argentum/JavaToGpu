...
- [x] Add OpenCL-from-`IrGpu` round-trip/parity tests for the current OpenCL workload slice.
  First parity guard is in place: OpenCL lowering checks that packaged `IrGpu` points at the same derived OpenCL source resource as the generated descriptor and fails fast on drift. The parity result also exposes `regeneration.*` readiness/blockers, making the current derived-source fallback explicit in lowering diagnostics. `OpenClIrGpuReconstructionPreview` adds a non-mutating reconstructability check that distinguishes transitional fallback artifacts from synthetic backend-neutral-ready artifacts without switching the default production source path, and runtime dumps emit `opencl-irgpu-reconstruction-preview.properties` plus backend-neutral `backend-source-reconstruction.properties` so CI artifacts can show attempted/reconstructable/ready/reconstructed status, selected source, payload format, entry emitted name, method-body count, source availability, blockers, diagnostics, and `sourceParity.*` comparison fields. Preview-only readiness is now explicit (`ready=true`, `reconstructed=false`, `sourceAvailable=false`) until `OpenClIrGpuSourceReconstructor` actually assembles a source payload; assembled payloads report `ready=true`, `reconstructed=true`, and `sourceAvailable=true`. `OpenClIrTextBodyParser` / `OpenClIrTextBodyEmitter` now cover the first practical `ir-text-v1` source-reconstruction slice: flat `var`, `set`, `return`, expression statements, helper calls, OpenCL intrinsic calls, OpenCL vector constructors through `init<T>(...)`, private local array declarations through `private-array`, escaped-line snapshot normalization, indented `if` / `else`, simple `for init=(...) cond=... update=(...)` loops, `while` / `do-while` loops, `switch` / `case` / `default` blocks, and `break` / `continue` / `loop-break`, with non-throwing unsupported-line blockers for anything outside that slice. `OpenClIrGpuSourceAssembler` can produce diagnostic reconstructed OpenCL source payloads for entry/helper artifacts with required metadata, and full-source emission/reconstruction tests now prove control-flow and expression-heavy bodies (helpers, casts, vector initializers, intrinsic templates, and ternary expressions) can round-trip through `IrGpu -> OpenCL source` with `sourceParity.matched=true`. Workload validation has reached parity/runtime-equivalence for the current five OpenCL workload kernels, including the image workload. Production switching is still intentionally blocked until the explicit promotion gate is enabled.
...
- [x] Add runtime dump evidence for backend source-switching decisions.
  Runtime artifact dumps now include `backend-source-switching-decision.properties`, which records the selected source mode, production-like profile detection, production source-switching flag, source-ready state, reconstruction availability, source-parity status, source-promotion status, and the resulting decision (`compile-descriptor-source`, `compile-irgpu-source-review`, `compile-irgpu-source-production`, `reject-irgpu-source-unavailable`, `reject-irgpu-source-parity`, or `reject-production-irgpu-source`). This keeps production rollout explanations separate from optimizer gates and workload promotion gates while fail-closing `opencl.sourceSelection=irgpu` when reconstructed source is missing, preview-ready but not assembled, or parity has not matched.
...
- [x] Route OpenCL lowering through the shared source-switching policy/decision.
  `OpenClBackendLowerer` now normalizes compile options through `GpuBackendSourceSwitchingPolicy` and gates explicit reconstructed-`IrGpu` source lowering through `GpuBackendSourceSwitchingDecision`. This keeps actual lowering decisions aligned with artifact dump diagnostics, preserves descriptor-source default behavior, and leaves production `IrGpu` source compilation fail-closed until production switching and accepted promotion evidence are both enabled.
