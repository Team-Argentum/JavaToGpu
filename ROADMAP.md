...
- [ ] Persist remaining extension participation in `IrGpu` and cross-pipeline artifacts.
  `kernel.irgpu.properties`, optimizer drift artifacts, promotion gates, validation history, and future backend artifacts should record the relevant extension ids, versions, phases, decisions, diagnostics, and whether each extension was advisory or production-affecting. Runtime compile dumps now include a first normalized `runtime-extension-participation.properties` aggregate for device-selection, runtime optimizer, and backend compiler-feedback execution reports, and OpenCL validation reports/history now aggregate those per-kernel artifacts into `Runtime Extension Participation` evidence. `IrGpu` artifacts now persist read-only IR Validator execution participation through `extensionParticipation.*` fields. Backend-lowerer, runtime-equivalence, promotion-gate, artifact-writer, and broader non-validator `IrGpu` participation still need the same normalized model. This keeps generated artifacts auditable when a project depends on company-specific plugins or third-party GPU libraries.
