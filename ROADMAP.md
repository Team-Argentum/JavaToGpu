...
- [ ] Fill real per-family runtime-equivalence payload contents before enabling any production rewrite.
  Persist full CPU/reference and pre/post optimization result payload files, tolerance metadata files, machine-readable diagnostics, and failed-evidence regression fixtures for each optimizer family selected for promotion. The current bridge exposes stable resource ids plus compact payload contents; future work must materialize durable files from real optimizer execution evidence before any production rewrite can be enabled.
