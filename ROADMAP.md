...
- [ ] Persist backend-neutral annotation intent in `IrGpu` storage.
  `kernel.irgpu.properties` should carry normalized annotation metadata separately from backend-specific raw attributes: kernel entry kind, address-space intent, work-group size hints/requirements, unroll policy, fast-math policy, backend/device compatibility, and any raw backend escape hatches. This prevents OpenCL-generated source from becoming the only source of truth and lets future CUDA/Vulkan/Metal lowerers consume the same `IrGpu` artifact without parsing OpenCL-specific strings.
