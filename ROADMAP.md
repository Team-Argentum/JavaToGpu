...
- [ ] Add register-pressure estimation and developer diagnostics.
  ...
  - [x] Persist launch-advisory counts in validation history. History properties and Markdown now retain a backward-compatible compact `kernelLaunchAdvisoryStatus` summary with kernel, aligned, non-preferred, driver-selected, unavailable, missing, and blocking counts so driver/workload launch-shape drift remains visible across runs.
  - [x] Classify launch-advisory drift against the latest compatible history entry. The reporter compares the same backend/device/vendor/lane across driver versions, emits signed count deltas, and marks increases in blocking, missing, unavailable, or non-preferred launches as `regressed`. Stable, changed, improved, no-baseline, and unavailable states remain explicit in the validation report and CI Markdown fragment.
