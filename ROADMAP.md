...
- [ ] Add a runtime IR peephole optimizer / InstCombine pass.
  Future work should add an opt-in `GpuRuntimeIrPeepholePass` family on top of `GpuRuntimeIrOptimizationPass` for local expression-tree rewrites such as `a + t * (b - a) -> mix(a, b, t)`, `a * b + c -> mad/fma(a, b, c)`, `min(max(x, lo), hi) -> clamp(x, lo, hi)`, ternary step forms -> `step(edge, x)`, and small fixed-width dot-product patterns. This belongs in I3 rather than I2 because it is a mutating runtime optimizer, not a read-only validator. The first implementation must stay diagnostic-only/fail-closed until runtime-equivalence, rollback, and A1/A2 evidence are stable.
- [ ] Add a safe IR pattern-matching and rewrite visitor for peephole passes.
  The pass should traverse `IrGpu` method bodies with a conservative matcher that can prove complete-pattern replacement before editing anything. Partial matches must produce diagnostics only. Rule implementations should be isolated (`mix`, `mad/fma`, `clamp`, `step`, `dot`) and report applied/skipped/blocked counts per rule so optimizer drift artifacts can detect regressions.
- [ ] Add frontend fast-math policy metadata for runtime optimizer decisions.
  Introduce a method-level opt-in policy such as `@GpuOptimize(fastMath = true)` only after deciding the default semantics. Recommended production-safe default is strict (`fastMath=false`) unless an existing profile or annotation explicitly enables fast-math, because `mad/fma`, reassociation, `mix`, and dot-product rewrites may change IEEE-754 rounding. Java source and ASM frontends should both preserve the policy in shared GPU/IR metadata instead of creating frontend-specific flags.
- [ ] Persist fast-math and optimizer-policy metadata in `IrGpu` storage.
  `kernel.irgpu.properties` should carry stable method/kernel metadata for fast-math, optimizer profile, and future rule-family toggles. Runtime optimizer passes must read this metadata from the loaded `IrGpu` artifact and disable unsafe rewrites when fast-math is false or absent.
