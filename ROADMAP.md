...
- [ ] Add startup device self-test and ranking for multi-device setups.
  Future runtime should run a small evidence-first device self-test suite when multiple GPUs are available, then rank devices by correctness, backend support, memory limits, throughput smoke, long-running stability, and known quirk profile. A setup such as RTX 5070 + RTX 3060 + Intel Arc 310 should normally assign most work to RTX 5070, secondary work to RTX 3060, and either exclude or rarely use Arc 310 unless user policy says otherwise. Self-test results should be cached with driver/runtime/version identity and invalidated when hardware, driver, backend, or compiler artifacts change.
- [ ] Add iGPU-specific validation buckets and adaptation policy.
  Integrated GPUs need their own validation expectations because unified memory, lower register budgets, shared bandwidth, and driver behavior differ from RTX 3060/5070 and RX 7800 XT baselines. Add an iGPU lane profile with smaller workloads, memory-transfer-sensitive tests, register-pressure-sensitive tests, and explicit pass/fail/skip evidence so Intel/AMD iGPU devices are not judged only by dGPU-sized stress assumptions. Until those artifacts exist, iGPU production promotion must remain advisory/fail-closed.
- [ ] Add register-pressure estimation and developer diagnostics.
  The compiler should estimate per-kernel register pressure from `IrGpu` plus backend/vendor limits and surface Rust-like diagnostics when a kernel is likely to spill or exceed a target device class. Diagnostics should explain the estimated register use, known or assumed hardware limit, hottest expression/temporary families, and suggested fixes. This should start as advisory telemetry and only become blocking under explicit strict optimizer or production profiles.
