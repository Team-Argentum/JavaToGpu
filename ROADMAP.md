...
- [ ] Fill real per-family runtime-equivalence payload contents before enabling any production rewrite.
  Runtime dumps now materialize a durable per-family/per-pass directory with a manifest, CPU/reference, pre-optimization, post-optimization, tolerance, failure-fixture, and machine-readable diagnostics properties files. The compact `runtime-optimizer-family-equivalence-payload.properties` index records every durable path, incomplete components remain explicit `not-recorded` files, and artifact writing rejects paths that escape the kernel output directory. This item remains open until compact payload summaries are replaced by full raw CPU/reference and pre/post result vectors from real optimizer execution evidence for each family selected for promotion.
