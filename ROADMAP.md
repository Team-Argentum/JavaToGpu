...
- [ ] Preserve fast-math policy through the ASM frontend.
  Structured/general ASM ingestion should write the same `IrGpuOptimizerPolicyMetadata` contract as Java source instead of adding an ASM-only flag. Missing ASM policy must resolve to `default-strict`; future ASM compile options may opt in only through the same shared metadata and evidence gates.
...
- [ ] Add proof, runtime-equivalence, and rollback gates for peephole rewrites.
  Peephole rewrites must emit `GpuRuntimeIrOptimizationProofArtifact` fields with optimizer family (`peephole`, `instcombine`, or per-rule families), rule counters, fast-math policy, original/transformed IR identities, and compact/durable runtime-equivalence payload evidence. Failed proofs, incomplete pattern rewrites, runtime-equivalence failures, or rollback diagnostics must keep the original IR selected.
- [ ] Surface peephole telemetry in optimizer drift and promotion artifacts.
  `runtime-optimizer-drift.properties`, workload gates, validation history, and production-promotion explainability should show applied/skipped/blocked counts for `mix`, `mad/fma`, `clamp`, `step`, and `dot`, plus first blocker diagnostics. Production promotion must remain disabled until these rule families have stable A1/A2 runtime evidence and backend/vendor confidence.
...
- [x] Allow third-party optimizer and peephole passes without core forks.
  External libraries can contribute full `GpuRuntimeIrOptimizationPass` implementations or typed-body `GpuRuntimeIrPeepholeRule` implementations through ServiceLoader. `GpuRuntimeIrPeepholeRuleRegistry` provides deterministic ordering, duplicate rule-id and extension-id rejection, phase/capability/permission validation, advisory failure isolation, and production-profile fail-closed behavior. Rule reports preserve rule id/version, extension id/version, method, candidate/proposal counts, proof status, diagnostics, and custom fields. The built-in MAD/FMA detector now uses the same public rule SPI. The current pass remains diagnostic/proposal-only: rules cannot bypass fast-math policy, proof requirements, production authorization, rollback, runtime-equivalence, or final IR selection gates.
- [ ] Add optional approval-bound IR optimizer modules.
  Keep `GpuIrValidationProvider` strictly read-only and introduce future optional IR optimizer modules as separate `MUTATION_PROPOSAL` providers that projects may omit entirely. An optimizer module should emit proposal artifacts plus an approval-template manifest instead of silently changing `IrGpu`; the project or downstream JAR then opts in by packaging an explicit approval manifest under a stable resource path such as `META-INF/javatogpu/ir-optimization-approvals/`. Approval must be identity-bound to rule id/version, method identity, original `IrGpu` SHA-256, proposed optimized `IrGpu` SHA-256, backend target, proof/runtime-equivalence status, and rollback/fallback evidence. Missing approval, `enabled=false`, stale hashes, failed proof, or backend/device mismatch must leave the selected IR as original and record `optimization.status=not-applied`. This gives projects a Git-approval-style review boundary for optimizer adoption without weakening the existing validator contract or forcing optional optimization dependencies on users who do not need them.
