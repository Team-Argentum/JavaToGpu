...
- [ ] Fill real per-family runtime-equivalence payload contents before enabling any production rewrite.
  Persist concrete CPU/reference and pre/post optimization result payloads, tolerance metadata, machine-readable diagnostics, and failed-evidence regression fixtures for each optimizer family selected for promotion. The current layer records payload presence/completeness only; future work must populate the actual payload resources from real optimizer execution evidence.
