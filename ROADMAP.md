...
- [x] Implement the first practical `ir-text-v1 -> OpenCL` source-reconstruction slice.
  `OpenClIrTextBodyParser` / `OpenClIrTextBodyEmitter` now cover flat `var`, `set`, `return`, expression statements, helper calls, OpenCL intrinsic calls, OpenCL vector constructors through `init<T>(...)`, private local array declarations through `private-array`, escaped-line snapshot normalization, indented `if` / `else`, simple `for init=(...) cond=... update=(...)` loops, `while` / `do-while` loops, `switch` / `case` / `default` blocks, and `break` / `continue` / `loop-break`, with non-throwing unsupported-line blockers for anything outside that slice. `OpenClIrGpuSourceAssembler` can assemble entry/helper artifacts with required metadata into diagnostic reconstructed OpenCL source.
...
- [ ] Promote reconstructed `IrGpu` source from review lane to production source switching.
  This remains intentionally blocked. Production source switching must require explicit `opencl.productionSourceSwitching=enabled`, accepted production optimizer/source-promotion gates, clean runtime-equivalence evidence, no fallback/rollback selection, and A1/A2 validation confidence before reconstructed `IrGpu` source can become the default production input.
