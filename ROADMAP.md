...
- [ ] Add register-pressure estimation and developer diagnostics.
  - [x] Add a backend-neutral typed-`IrGpu` pressure estimator. `GpuRuntimeRegisterPressureAnalyzer` conservatively tracks method parameters, block-live local values, vector lane weights, private arrays, and Sethi-Ullman-like expression peaks without claiming backend compiler accuracy.
  ...
  - [x] Add synthetic low/moderate/critical/unavailable regression coverage, including vector-heavy dGPU and private-array iGPU fixtures.
  - [ ] Add precise last-use/liveness reconstruction across nested scopes, helper inlining boundaries, loops, and control-flow joins. The current model intentionally keeps declarations live for their containing block and can overestimate pressure.
  - [ ] Ingest backend compiler register/occupancy reports when available. OpenCL vendor logs, CUDA ptxas/NVRTC metadata, Vulkan/SPIR-V tooling, and Metal compiler diagnostics should refine or replace the heuristic while preserving the same artifact contract.
