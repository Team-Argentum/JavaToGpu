...
- [ ] Persist remaining extension participation in `IrGpu` and cross-pipeline artifacts.
  `kernel.irgpu.properties`, optimizer drift artifacts, promotion gates, validation history, and future backend artifacts should record the relevant extension ids, versions, phases, decisions, diagnostics, and whether each extension was advisory or production-affecting. Runtime compile dumps now include a normalized `runtime-extension-participation.properties` aggregate for build-time `IrGpu` validator metadata, device-selection, runtime optimizer, and backend compiler-feedback execution reports. OpenCL workload gates, workload summaries, workload-summary CLI output, validation reports, and validation history now surface per-kernel and aggregate `Runtime Extension Participation` evidence, including source breakdowns such as `original-irgpu:ir-validation`, `runtime-ir-optimization`, and `backend-compiler-feedback`. Backend-lowerer, runtime-equivalence, artifact-writer, and future backend-specific artifacts still need the same normalized participation model. This keeps generated artifacts auditable when a project depends on company-specific plugins or third-party GPU libraries.
