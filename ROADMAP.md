...
- [ ] Extend backend lowering to consume full `IrGpu` artifacts.
  OpenCL lowering still uses the existing generated source as the source payload. A reconstruction plan now records whether OpenCL can select backend-neutral `IrGpu` source or must stay on the transitional fallback, and `GpuBackendLowerer.sourceSelectionPlan(...)` exposes that backend-neutral source-selection decision before module lowering. `OpenClIrGpuReconstructionPreview` provides the first non-mutating runtime contract for checking method-body count, entry emitted name, selected source path, and blockers before direct reconstruction is enabled; runtime dumps preserve the same state in `opencl-irgpu-reconstruction-preview.properties` and `backend-source-reconstruction.properties`. `OpenClIrGpuSourceEmission` now runs parser-backed inspection, `OpenClIrTextBodyEmitter` lowers simple flat `var` / `set` / `return` bodies into OpenCL-C body text, and `OpenClIrGpuSourceAssembler` assembles entry signatures plus emitted body text into a complete diagnostic OpenCL source for simple entry-only artifacts with entry parameter metadata. The backend module artifact still consumes the backend-neutral reconstruction result for `sourceOrigin` / `runtimeLoadMode` while using generated OpenCL source as the production payload, so diagnostics can now distinguish descriptor-source fallback, derived OpenCL fallback, missing parameter metadata, unsupported helpers/control-flow, and the future promotable `IrGpu` source path without changing runtime behavior. Next source-generation work should add helper signature/body assembly and expand the parser/emitter beyond flat statements, then compare reconstructed OpenCL against generated descriptor source before enabling runtime promotion.
