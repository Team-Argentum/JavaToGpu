...
- [ ] Add explicit OpenCL device discovery and selection policy.
  The OpenCL runtime must stop relying on driver/default-device ordering when more than one device is present. Device discovery should enumerate all platforms/devices, classify discrete GPU vs iGPU/CPU/unknown devices, rank candidates by user policy and capability, and record a deterministic selected-device artifact. By default, discrete GPUs such as RTX/Radeon/Arc dGPU should outrank integrated GPUs, while explicit user selection by vendor/device name/index/UUID must override auto-selection. If the runtime rejects an iGPU because a stronger dGPU exists, the decision should be visible in diagnostics instead of silently choosing the first OpenCL device.
- [ ] Add method-level device constraints and fallback variants.
  Future frontends should support annotations/metadata that declare which backend, vendor family, device class, or feature set a method supports, especially for native `@CCode` / pure C bodies that may only be valid on a subset of devices. Runtime device selection must reject methods with no compatible selected device and produce a clear diagnostic instead of compiling unsupported code. The model should also support fallback method variants, for example one implementation optimized for dGPU and another for iGPU, with deterministic selection based on device policy, required features, and user overrides.
...
- [ ] Persist method device constraints and fallback groups in `IrGpu` storage.
  `kernel.irgpu.properties` should carry stable per-method compatibility metadata such as supported backends, supported vendors, supported device classes (`dgpu`, `igpu`, `cpu-opencl`), required capabilities, excluded devices, fallback group id, fallback priority, and native-code compatibility notes. Java source, ASM, and native C helper paths should all write the same metadata so runtime planning can choose the correct method variant without frontend-specific logic.
- [ ] Add user-catchable GPU runtime unavailability exceptions.
  Generated GPU method wrappers should surface a stable runtime exception hierarchy when no compatible backend/device/method variant exists, when OpenCL/CUDA/Vulkan/Metal initialization fails, when method-level device constraints reject the selected device, or when runtime compile options / compile arguments are invalid for the selected backend. This allows user code to catch a library exception such as `UnsupportedGpuException` / `GpuRuntimeUnavailableException` and call a CPU or custom fallback without depending on backend-specific exception text. The exception should include the method name, backend target, selected or rejected devices, first incompatibility reason, original backend cause, and a short Rust-like diagnostic summary.
- [ ] Preserve user fallback semantics around generated GPU method invocations.
  Because annotated GPU methods are rewritten/wrapped during build, the generated invocation path must not swallow user `try/catch` blocks, move GPU setup outside the user's guarded region, or replace catchable runtime failures with unchecked low-level backend errors. A user pattern such as `try (GpuRuntimeScope scope = GpuRuntime.useOpenClSharedCache()) { Kernel.method(...); } catch (UnsupportedGpuException e) { Fallback.invoke(); }` should remain valid after code generation even when `Kernel.method(...)` is replaced with the generated GPU launcher call. Generated launchers should translate device-selection, method-compatibility, compile-option, backend initialization, runtime setup, and kernel compilation failures into the stable GPU exception hierarchy while preserving original causes for debugging.
- [ ] Add build-time wrapper diagnostics for catchable GPU failures.
  The method-rewrite layer should keep an explicit mapping from the original annotated `@GPU` call site to the generated launcher invocation, then attach that source anchor to catchable runtime errors. If the generated wrapper fails before kernel execution, diagnostics should report the original Java call expression, the active runtime scope/backend, the selected or rejected device, the compile arguments, and the suggested fallback contract. This keeps errors understandable for code such as `try (GpuRuntimeScope scope = GpuRuntime.useOpenClSharedCache()) { GpuShowcase.basicMath(input, output); } catch (UnsupportedGpuException e) { Fallback.invoke(); }` instead of exposing only generated-code internals.
...
- [ ] Generalize physical discovery policy integration beyond default OpenCL startup.
  Future CUDA/Vulkan/Metal discovery should reuse the same pre-context selection boundary. OpenCL still needs explicit user overrides and request-specific re-evaluation when method compatibility or non-default compile arguments can change the valid device set.
